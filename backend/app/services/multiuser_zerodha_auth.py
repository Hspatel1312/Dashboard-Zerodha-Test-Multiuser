            ...
            if os.path.exists(self.access_token_file):
                try:
                    ...
                    if access_token:
                        ...
                        self.kite = kite
                        print(f"[INFO] User {self.user.username} - Zerodha Profile: {self.zerodha_profile_name}")
                        return kite
                except Exception as e:
                    print(f"[WARNING] User {self.user.username} - Saved token invalid: {str(e)}")

            # MANUAL AUTHENTICATION ONLY - Require request token
            if not manual_request_token:
                raise Exception("Manual request token required. Please use the login URL to get request token.")

            print(f"[INFO] User {self.user.username} - Using manual request token")
            try:
                ...
                self._last_auth_attempt = datetime.now()
                self.kite = kite
                print(f"[SUCCESS] User {self.user.username} - Manual auth successful: {self.zerodha_profile_name}")
                return kite
            except Exception as e:
                print(f"[ERROR] User {self.user.username} - Manual authentication failed: {str(e)}")
                raise Exception(f"Manual authentication failed: {str(e)}")
        
        except Exception as e:
            error_msg = f"[ERROR] User {self.user.username} - Zerodha authentication failed: {str(e)}"
            print(error_msg)
            self._authenticated = False
            self.kite = None
            self.zerodha_profile_name = None
    ...
    def validate_existing_token(self):
        ...
        try:
            if not os.path.exists(self.access_token_file):
                print(f"[INFO] User {self.user.username} - No access token file found")
                return False
            ...
            if not access_token:
                print(f"[INFO] User {self.user.username} - Access token file is empty")
                return False
            
            print(f"[INFO] User {self.user.username} - Validating existing access token...")
            ...
            try:
                ...
                print(f"[SUCCESS] User {self.user.username} - Token valid, Profile: {self.zerodha_profile_name}")
                return True
                
            except Exception as e:
                print(f"[ERROR] User {self.user.username} - Token invalid: {str(e)}")
            ...
            print(f"[ERROR] User {self.user.username} - Error validating token: {str(e)}")
            return False
