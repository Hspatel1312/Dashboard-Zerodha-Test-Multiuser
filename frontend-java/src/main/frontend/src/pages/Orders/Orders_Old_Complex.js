...
// Hooks
import { 
  useUserSystemOrders as useSystemOrders,
  useUserFailedOrders as useFailedOrders,
  useUserLiveOrders as useLiveOrders,
  useUserOrdersWithRetries as useOrdersWithRetries,
  useUserMonitoringStatus as useMonitoringStatus,
  useUserRetryFailedOrdersMutation as useRetryFailedOrdersMutation,
  useUserResetSystemOrdersMutation as useResetSystemOrdersMutation, 
  useUserUpdateLiveOrderStatusMutation as useUpdateLiveOrderStatusMutation,
  useStartOrderMonitoringMutation,
  useStopOrderMonitoringMutation,
  useUpdateOrderStatusFromZerodhaMutation,
...
const Orders = () => {
  ...
  const startMonitoringMutation = useStartOrderMonitoringMutation();
  const stopMonitoringMutation = useStopOrderMonitoringMutation();
  const updateFromZerodhaMutation = useUpdateOrderStatusFromZerodhaMutation();

  const orders = systemOrders?.data || [];
  ...
  console.log('DEBUG monitoringStatus:', monitoringStatus);
  console.log('DEBUG monitoringStatus.data:', monitoringStatus?.data);
  const monitoringActive = monitoringStatus?.data?.monitoring_status?.monitoring_active || false;
  const pendingOrdersCount = monitoringStatus?.data?.monitoring_status?.pending_orders || 0;
  const monitoringStatusMessage = monitoringStatus?.data?.monitoring_status?.status_message || 'Checking monitoring status...';
  ...
  const getCombinedOrders = () => {
    return orders.map(systemOrder => {
      ...
      let statusMessage = systemOrder.failure_reason || systemOrder.error;
  ...
  const handleUpdateAllFromZerodha = () => {
    updateFromZerodhaMutation.mutate();
  };
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            ...
            {/* Order Status Sync Button */}
            <Tooltip title="Update all order status from Zerodha">
              <Button
                variant="contained"
                startIcon={<SyncIcon />}
                onClick={handleUpdateAllFromZerodha}
                disabled={updateFromZerodhaMutation.isLoading}
                sx={{
              ...
              >
                {updateFromZerodhaMutation.isLoading ? 'Syncing...' : 'Sync Status'}
              </Button>
            </Tooltip>

            {/* Monitoring Control Buttons */}
            {monitoringActive ? (
              <Tooltip title="Stop automatic order monitoring">
                <Button
                  variant="outlined"
                  startIcon={<StopIcon />}
                ...
                >
                  {stopMonitoringMutation.isLoading ? 'Stopping...' : 'Stop Monitor'}
                </Button>
              </Tooltip>
            ) : (
              <Tooltip title="Start automatic order monitoring">
                <Button
                  variant="contained"
                  startIcon={<StartIcon />}
                  onClick={handleStartMonitoring}
                  disabled={startMonitoringMutation.isLoading}
                  sx={{
                    background: 'linear-gradient(135deg, #10B981 0%, #059669 100%)',
                    color: 'white',
                    '&:hover': {
                ...
                >
                  {startMonitoringMutation.isLoading ? 'Starting...' : 'Start Monitor'}
                </Button>
              </Tooltip>
            )}
          ...
          >
            <CardContent sx={{ py: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                {monitoringActive ? <StartIcon /> : <StopIcon />}
                Monitoring
              </Typography>
              <Typography variant="h6" sx={{ fontWeight: 700 }}>
                {monitoringActive ? '🟢 ACTIVE (FIXED)' : '🔴 Inactive'}
              </Typography>
